...
echo "<h1 class=\"title-scoreboard flow-text\">Balloon Status</h1>\n\n";

foreach ($cdatas as $cdata) {
	if ( isset($cdata['freezetime']) &&
	     difftime($cdata['freezetime'], now()) <= 0
	) {
		echo "<h4 class=\"timeTest flow-text\">Scoreboard of c${cdata['cid']} (${cdata['shortname']}) is now frozen.</h4>\n\n";
	}
}
...
if ( !empty($BALLOONS) ) {
	...
	echo "<table class=\"white z-depth-1 striped list sortable balloons\">\n<thead>\n" .
