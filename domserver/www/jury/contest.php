...
if ( !empty($_GET['cmd']) ):
	...
	// echo "<h2 style=\"font-size: 30px;\" class=\"title-scoreboard flow-text\">$title</h2>\n\n";
	echo '<div class="row" style="margin: 0">';
	echo "<div style='margin-bottom: 10px;' class='col s8 offset-s2'>";
    echo "<h2 class=\"title-scoreboard flow-text\">$title</h2>\n\n";
    echo "</div>";
    echo "</div>";
	echo '<div class="row" style="margin: 0">';
    echo '<div class="col s8 offset-s2 white">';

	echo addForm('edit.php');

	echo "<table class='z-depth-2'>\n";

	if ( $cmd == 'edit' ) {
		$row = $DB->q('MAYBETUPLE SELECT * FROM contest WHERE cid = %s', $id);
		if ( !$row ) error("Missing or invalid contest id");

		echo "<tr><td class='tb-add'>Contest ID:</td><td>" .
...
<tr><td style='' class='tb-add'><label for="data_0__shortname_">Short name:</label></td>
<td class="pd-add" colspan="2"><?php echo addInput('data[0][shortname]', @$row['shortname'], 40, 10, 'required')?></td></tr>
<tr><td class='tb-add'><label for="data_0__name_">Contest name:</label></td>
<td class="pd-add" colspan="2"><?php echo addInput('data[0][name]', @$row['name'], 40, 255, 'required')?></td></tr>
<tr><td class='tb-add'><label for="data_0__activatetime_string_">Activate time:</label></td>
<td class="pd-add" style="width: 70%"><?php echo addInput('data[0][activatetime_string]', (empty($row['activatetime_string'])?strftime('%Y-%m-%d %H:%M:00 ').date_default_timezone_get():$row['activatetime_string']), 30, 64, 'required pattern="' . $pattern_dateorneg . '"')?></td>
<td  rowspan="6">
<blockquote>
...
<tr><td class='tb-add'><label for="data_0__starttime_string_">Start time:</label></td>
<td class="pd-add"><?php echo addInput('data[0][starttime_string]', @$row['starttime_string'], 30, 64, 'required pattern="' . $pattern_datetime . '"')?></td></tr>

<tr><td class='tb-add'><label for="data_0__freezetime_string_">Scoreboard freeze time:</label></td>
<td class="pd-add"><?php echo addInput('data[0][freezetime_string]', @$row['freezetime_string'], 30, 64, 'pattern="' . $pattern_dateorpos . '"')?></td></tr>

<tr><td class='tb-add'><label for="data_0__endtime_string_">End time:</label></td>
<td class="pd-add"><?php echo addInput('data[0][endtime_string]', @$row['endtime_string'], 30, 64, 'required pattern="' . $pattern_dateorpos . '"')?></td></tr>

<tr><td class='tb-add'><label for="data_0__unfreezetime_string_">Scoreboard unfreeze time:</label></td>
<td class="pd-add"><?php echo addInput('data[0][unfreezetime_string]', @$row['unfreezetime_string'], 30, 64, 'pattern="' . $pattern_dateorpos . '"')?></td></tr>

<tr><td class='tb-add'><label for="data_0__deactivatetime_string_">Deactivate time:</label></td>
<td class="pd-add"><?php echo addInput('data[0][deactivatetime_string]', @$row['deactivatetime_string'], 30, 64, 'pattern="' . $pattern_dateorpos . '"')?></td></tr>

<tr><td class='tb-add'>Process balloons:</td><td class="pd-add">
...
<tr><td class='tb-add'>Public:</td><td class="pd-add">
...
<tr><td class='tb-add'>Enabled:</td><td class="pd-add">
...
<table class="striped z-depth-2" id="problems_table">
	<thead>
	<tr>
		<th >ID</th>
		<th>name</th>
...
echo "<table class='striped z-depth-2'>\n";
