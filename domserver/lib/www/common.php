...
function putSubmissions($cdatas, $restrictions, $limit = 0, $highlight = null)
{
	...
	// table header
	echo "<table class=\"list white z-depth-1 striped sortable\">\n<thead>\n<tr>" .
...
function putTeam($teamid) {
	...
	echo "<h1 class=\"title-scoreboard flow-text\">Team ".specialchars($team['name'])."</h1>\n\n";
...
function putClock() {
	...
	?>
	<div class="col s9">
		<img style="width: 90%;height: auto;" src="../images/2.png" alt='' />
	</div>
	<div class="col s3">
	...
	if ( IS_JURY || count($cdatas) > 1 ) {
		echo "<div class=\"\" id=\"selectcontest\">\n";
		...
		echo '<span style="font-size: 17px;" class="card-title">Kì thi:</span>' . addSelect('cid', $values, $cid, true);
...
";
	...
	<div class="chip">
	    <img src="../images/abc.png" alt="Contact Person">
		<div id="timeleft">
		    <?php echo "<span class='i'>Time left:</span>" .$left; ?>
		</div>
	...
	if ( logged_in() ) {
		echo "<div style='font-size: 15px;padding-bottom: 10px;' class=\"flow-text center-align\" id=\"username\">logged in as " . $username.'  || '
			. ( have_logout() ? " <a style='text-decoration: underline;' href=\"../auth/logout.php\"> Đăng xuất</a>" : "" )
			. "</div>";
	}
...
function putDOMjudgeVersion() {
	echo "<address class=\"flow-text center-align row\">DOMjudge/" . DOMJUDGE_VERSION .
...
function putProblemTextList()
{
	...
	if ( ! have_problemtexts() ) {
		...
		if ( $res->count() > 0 ) {
			echo "<table class=\"z-depth-1 striped centered\">\n";
			?>
			<thead>
			<tr>
				<th class="center-align">ID</th>
				<th class="center-align">Hình ảnh</th>
				<th class="center-align">Tên bài tập</th>
			</tr>
			</thead>
			<?php
			while($row = $res->next()) {
				print '<tr> ' .
					  '<td>'.urlencode($row['probid']).'</td>'.	
				      '<td><img src="../images/' . urlencode($row['problemtext_type']) .

				      '.png" alt="' . specialchars($row['problemtext_type']) .

				      '" /></td> <td><a href="problem.php?id=' . urlencode($row['probid']) . '">' .

				      'Problem ' . specialchars($row['shortname']) . ': ' .

				      specialchars($row['name']) . "</a></td></tr>\n";
			}
			echo "</table>\n";
		}
	}
