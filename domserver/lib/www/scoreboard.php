	...
	$limitteams = null, $displayrank = TRUE, $center = FALSE, $showlegends = TRUE)
{
	...
	echo '<table class="white z-depth-1 scoreboard highlight centered scoretable-custom' . (IS_JURY ? ' scoreboard_jury' : '') . ($center ? ' center' : '') . "\">\n";
	...
	echo '<tr class="scoreheader">' .
		'<th class="th-custom" title="rank" scope="col">' . jurylink(null,'rank') . '</th>' .
		'<th class="th-custom" title="team name" scope="col"' .
		( $SHOW_AFFILIATIONS ? ' colspan="2"' : '' ) .
		'>' . jurylink(null, 'team') . '</th>' .
		'<th class="th-custom" title="# solved / penalty time" colspan="2" scope="col">' .
		jurylink(null, 'score') . '</th>' . "\n";
	foreach( $probs as $pr ) {
		echo '<th class="th-custom" title="problem \'' . specialchars($pr['name']) . '\'" scope="col">';
		$str = specialchars($pr['shortname']) .
		       (!empty($pr['color']) ? ' <div class="circle circle-custom" style="background: ' .
			specialchars($pr['color']) . ';"></div>' : '') ;

		if ( !$static && (IS_JURY || $pr['hastext']>0) ) {
			echo '<a class="scoreboard-headlink" href="problem.php?id=' . urlencode($pr['probid']) .
				'">' . $str . '</a>';
		} else {
			echo '<a>' . $str . '</a>';
		}
	...
	foreach( $scores as $team => $totals ) {
		...
		echo '><td class="scorepl td-custom">';
		...
		if ( $SHOW_AFFILIATIONS ) {
			echo '<td class="scoreaf td-custom">';
		...
		echo
			...
			'</td>';
		echo
			'<td class="scorenc td-custom">' . jurylink(null,$totals['num_points']) . '</td>' .
			'<td class="scorett td-custom">' . jurylink(null,$totals['total_time'] ) . '</td>';
		...
		foreach ( array_keys($probs) as $prob ) {
			...
			if( $matrix[$team][$prob]['is_correct'] ) {
				...
				echo '"td-custom score_pending"';
			} elseif ( $matrix[$team][$prob]['num_submissions'] > 0 ) {
				echo '"td-custom score_incorrect"';
			} else {
				echo '"td-custom score_neutral"';
			}
	...
	if ( $showlegends ) {
		...
		if ( empty($limitteams) && count($categs) > 1 ) {
			echo "<div class=\"row\"> <div class=\"col s6\"> <table id=\"categ_legend\" class=\"z-depth-1 white scoreboard scorelegend categ_legend-custom" .
			    (IS_JURY ? ' scoreboard_jury' : '') . "\">\n" .
			    "<thead><tr><th scope=\"col\">" .
			    jurylink('team_categories.php','Danh mục') .
			    "</th></tr></thead>\n<tbody>\n";
		...
		echo "<table id=\"cell_legend\" class=\"z-depth-1 white scoreboard scorelegend cell_legend-custom striped" .
		    (IS_JURY ? ' scoreboard_jury' : '') . "\">\n" .
...
<?php
	...
	echo "<h1 class=\"title-scoreboard flow-text\">Score Board " . specialchars($cdata['name']) . "</h1>\n\n";
	...
	if ( $fdata['showfinal'] ) {
		...
		echo "<h4 class=\"timeTest flow-text\">starts: " . printtime($cdata['starttime']) .
	...
	if ( $filter!==FALSE && $static!==TRUE ) {
		...
		?>
<table class="z-depth-1 white scorefilter">
<tr>
<td><a class="collapse pink-text" href="javascript:collapse('filter')">
<i style="font-size: 25px;" class="fa fa-filter" alt="filter&hellip;" title="filter&hellip;" aria-hidden="true"></i>

<!-- <img src="../images/filter.png" alt="filter&hellip;" title="filter&hellip;" class="picto" /> -->
</a>
<?= $filtertext ?>
</td>
...
// -->
	...
	echo "<div class='flow-text center-align row' style='padding-top: 50px; font-size:15px;' id=\"lastmod\">Last Update: $lastupdate \n" .
	     "using <a target=\"_blank\" href=\"https://www.domjudge.org/\">DOMjudge</a></div>\n\n";
